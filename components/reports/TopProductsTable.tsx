      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-zinc-100">
            ...
            <th className="pb-2 text-right text-xs font-medium text-zinc-500">{REPORTS.UNITS_SOLD}</th>
            <th className="pb-2 text-right text-xs font-medium text-zinc-500">{REPORTS.REVENUE}</th>
          </tr>
        </thead>
        <tbody>
          {data.map((row, i) => (
            <tr key={row.productId ?? row.name} className="border-b border-zinc-50 hover:bg-zinc-50">
              ...
              <td className="py-2.5 font-medium text-zinc-900 truncate max-w-[200px]">
                {row.name}
              </td>
              <td className="py-2.5 text-right text-zinc-600 tabular-nums">{row.totalQty.toLocaleString('en-NG')}</td>
              <td className="py-2.5 text-right font-medium text-zinc-900 tabular-nums">
